...
export default function AdminEdit() {
  ...
  useEffect(() => {
    getComponent(id)
      .then((c) => {
        ...
        const type = SLUG_TO_TYPE[c.category_slug] || "button";
        const parsed = c.yaml
          ? parsePowerAppsYAMLToSettings(c.yaml, type, c.name)
          : defaultSettings(c.category_slug);
        setSettings(parsed);
      })
  ...
  const handleSubmit = async (e) => {
    ...
    setError(null);
    try {
      await updateComponent(id, { ...form, yaml });
      navigate("/admin");
    } catch (err) {
