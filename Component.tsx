...
const appId = typeof __app_id !== 'undefined' ? __app_id : 'powerapps-yaml-manager';
...
export default function App() {
  ...
  const [catalogs, setCatalogs] = useState(['Navigation', 'Forms', 'Charts', 'Buttons', 'Containers']);
  
  const [activeTab, setActiveTab] = useState('all'); // 'all' or catalog name
  ...
  useEffect(() => {
    ...
    const compRef = collection(db, 'artifacts', appId, 'public', 'data', 'components');
    ...
    const unsubProjs = onSnapshot(projRef, (snap) => {
  ...
  const handleSaveComponent = async (data) => {
    if (!user) return;
    try {
      const colRef = collection(db, 'artifacts', appId, 'public', 'data', 'components');
      if (data.id) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'components', id));
  ...
  const handleAddToProject = async (componentId, projectId) => {
    try {
      ...
      const updatedComponents = [...(proj.componentIds || []), componentId];
      await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'projects', projectId), {
        componentIds: Array.from(new Set(updatedComponents))
      });
  ...
  const ComponentModal = ({ component, isOpen, onClose }) => {
    const [formData, setFormData] = useState(component || { name: '', catalog: catalogs[0], yaml: '' });
      ...
      <aside className="w-64 border-r border-slate-800 flex flex-col bg-slate-900/50">
        ...
        <nav className="flex-1 overflow-y-auto p-4 space-y-1">
          ...
          {catalogs.map(cat => (
            <button 
              key={cat}
              onClick={() => { setActiveTab(cat); setSelectedProjectId(null); }}
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-bold transition-all border ${activeTab === cat && !selectedProjectId ? 'bg-blue-600/10 text-blue-400 border-blue-500/20 shadow-lg' : 'hover:bg-slate-800 text-slate-400 border-transparent'}`}
            >
              <ChevronRight size={14} className={`transition-transform duration-300 ${activeTab === cat ? 'rotate-90 text-blue-400' : ''}`} />
              {cat}
            </button>
          ))}
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-900/20">
           <div className="flex items-center gap-3 px-2">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] text-slate-500 truncate font-mono">{user?.uid}</p>
              </div>
           </div>
        </div>
      </aside>
          ...
          {selectedComponent && (
            <div className="w-full lg:w-[600px] border-l border-slate-800 bg-slate-900 flex flex-col shadow-2xl relative">
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar">
                ...
                {/* Project Assignment Section - NEW Prominent Placement */}
